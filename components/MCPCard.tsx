...
import { formatDate, formatNumber, getHealthStatus } from '@/lib/utils'
...
export default function MCPCard({ mcp, isSelected, onToggle, onViewDetails }: MCPCardProps) {
  const healthStatus = mcp.healthStatus || getHealthStatus(mcp.id)
  const isFeatured = mcp.isFeatured
    ...
    <div
      className={`relative bg-white dark:bg-gray-800 rounded-xl border-2 p-6 cursor-pointer transition-all hover:shadow-lg ${
        isSelected
          ...
          : isFeatured
          ? 'border-blue-500 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 glow-pulse'
          : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'
      }`}
      onClick={() => onViewDetails(mcp)}
    >
      {/* Featured Badge - Von Restorff Effect */}
      {isFeatured && (
        <div className="absolute top-3 right-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-xs font-bold px-2 py-1 rounded-full">
          ⚡ Staff Pick
        </div>
      )}

      {/* Health Status Badge - Halo Effect */}
      ...
      <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400 mt-4">
        ...
        {mcp.upvotes !== undefined && (
          <div className="flex items-center gap-1">
            ...
            <span>{formatNumber(mcp.upvotes)} upvotes</span>
          </div>
        )}
        {mcp.rating && (
          <div className="flex items-center gap-1">
            <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
            <span>{mcp.rating.toFixed(1)} ({mcp.reviewCount || 0} reviews)</span>
          </div>
        )}
        {mcp.lastUpdated && (
          <div className="flex items-center gap-1">
